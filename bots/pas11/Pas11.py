...
class MyBot:
    ...
    def pathfinding(self, ants, loc1, loc2):
        ...
        while openset:
            ...
            f_score.sort()
            for _dist, x in f_score:
                if x in openset:
                    break
            if (
                x == loc2
                or n > self.depth
                or (ants.time_remaining() < 100 and len(came_from) > 1)
            ):
    ...
    def do_turn(self, ants):
        ...
        def do_move_direction(loc, direction):
            ...
            safe = 1
            if (
                len(ants.enemy_ants()) > len(ants.my_ants()) / 10
                and ants.turns_remaining() > 75
            ) and new_loc in danger_list:
                ...
                if len(nearby_ants[danger_list[new_loc]]) < 2:
                    ...
                    for other_ant_new_loc in nearby_ants[danger_list[new_loc]]:
                        if (
                            other_ant_new_loc not in orders
                            and other_ant_new_loc in potential_orders
                        ):
                            orders[other_ant_new_loc] = potential_orders[
                                other_ant_new_loc
                            ]

            if (
                ants.unoccupied(new_loc)
                and new_loc not in orders
                and new_loc not in potential_orders
            ):
                if safe:
                    orders[new_loc] = loc
                    return True
        ...
        for ant_loc, _owner in ants.enemy_ants():
            for a in range(-2, 3):
        ...
        for hill_loc, _hill_owner in ants.enemy_hills():
        ...
        for dist, ant_loc, hill_loc in ant_hill_dist:
            if (
                dist < 7
                and ant_loc not in orders.values()
                and len(ants.my_ants()) > len(ants.my_hills()) * 7
            ):
        ...
        ant_dist.sort()
        if ant_dist:
            for _dist, ant_loc, food_loc in ant_dist:
        ...
        for hill_loc in ants.my_hills():
            ...
            for enemy_loc, _owner in ants.enemy_ants():
                dist = ants.distance(enemy_loc, hill_loc)
                if dist < 20:
                    hill_nearby_ants[hill_loc] += 1
        # form formation around hill
        if (
            len(ants.enemy_ants()) > len(ants.my_ants()) / 20
            and len(ants.my_ants()) > len(ants.my_hills()) * 10
        ) or len(ants.my_ants()) > len(ants.my_hills()) * 20:
            for hill_loc in ants.my_hills():
        ...
        if (
            len(ants.enemy_ants()) > len(ants.my_ants()) / 10
            and len(ants.my_ants()) > len(ants.my_hills()) * 20
        ) or len(ants.my_ants()) > len(ants.my_hills()) * 30:
            for hill_loc in ants.my_hills():
                if hill_nearby_ants[hill_loc] > 3:
                    ...
                    ants.destination(ants.destination(hill_loc, "n"), "n")
                    ants.destination(ants.destination(hill_loc, "e"), "e")
                    ants.destination(ants.destination(hill_loc, "s"), "s")
                    ants.destination(ants.destination(hill_loc, "w"), "w")
                    corners = [c1, c2, c3, c4]
                    for pos in corners:
                        if ants.unoccupied(pos):
                            ant_dist = []
        ...
        # HILL ATTACK
        if (
            len(ants.my_ants()) > len(ants.enemy_ants()) * 2
            or len(ants.my_ants()) > len(ants.my_hills()) * 40
        ):
            for _dist, ant_loc, hill_loc in ant_hill_dist:
                if ant_loc not in orders.values():
                    do_move_location(ant_loc, hill_loc)
        ...
        ant_dist.sort()
        if ant_dist:
            for _dist, ant_loc, flag_loc in ant_dist:
        ...
        if ants.time_remaining() < 200 or self.depth > 2000 / len(ants.my_ants()):
            self.depth = int(self.depth * 0.8) + 1
