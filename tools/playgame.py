...
def open_log(path, mode="w"):
    """Open a log file that lives for the game and is closed later."""
...
stderr = Colorize(sys.stderr) if colorize else sys.stderr
...
def main(argv):
    ...
    try:
        if opts.profile:
            ...
            print(
                f"Running profile and outputting to {prof_file}",
                file=stderr,
            )
...
def run_rounds(opts, args):
    ...
    def get_cmd_name(cmd):
        ...
        for _i, part in enumerate(reversed(cmd.split())):
    ...
    if opts.player_seed is not None:
        game_options["player_seed"] = opts.player_seed
    if opts.engine_seed is not None:
    ...
    for round in range(opts.rounds):
        ...
        with open(opts.map) as map_file:
        ...
        if game.num_players != len(bots):
            if game.num_players > len(bots) and opts.fill:
                ...
                print(
                    f"Incorrect number of bots for map.  Need {game.num_players}, got {len(bots)}",
                    file=stderr,
                )
                for arg in args:
                    print(f"Bot Cmd: {arg}", file=stderr)
                break
        bot_count = len(bots)
        ...
        if opts.log_replay:
            if opts.log_dir:
                replay_path = os.path.join(opts.log_dir, f"{game_id}.replay")
                engine_options["replay_log"] = open_log(replay_path, "w")
            if opts.log_stdout:
        ...
        if opts.log_stream:
            if opts.log_dir:
                engine_options["stream_log"] = open_log(
                    os.path.join(opts.log_dir, f"{game_id}.stream"), "w"
                )
            if opts.log_stdout:
        ...
        if opts.log_input and opts.log_dir:
            engine_options["input_logs"] = [
                open_log(
                    os.path.join(opts.log_dir, f"{game_id}.bot{i}.input"),
                    "w",
                )
        ...
        if opts.log_output and opts.log_dir:
            engine_options["output_logs"] = [
                open_log(
                    os.path.join(opts.log_dir, f"{game_id}.bot{i}.output"),
                    "w",
                )
                for i in range(bot_count)
            ]
                ...
                if opts.log_stdout:
                    engine_options["error_logs"] = [
                        Tee(
                            Comment(stderr),
                            open_log(
                                os.path.join(opts.log_dir, f"{game_id}.bot{i}.error"),
                                "w",
                            ),
                        )
                        for i in range(bot_count)
        ...
        if opts.rounds > 1:
            print(f"# playgame round {round}, game id {game_id}")
        ...
        run_game(game, bots, engine_options)
        ...
        if replay_path:
            if opts.nolaunch:
                ...
                if opts.html_file is None:
                    visualizer.visualize_locally.launch(
                        replay_path, generated_path=f"replay.{game_id}.html"
                    )
                else:
                    visualizer.visualize_locally.launch(
