...
class Ants:
    ...
    def setup(self, data):
        ...
        self.map = [[LAND for col in range(self.width)] for row in range(self.height)]

    def update(self, data):
        # clear ant and food data
        for (row, col), _owner in self.ant_list.items():
        ...
        for row, col in self.dead_list:
            self.map[row][col] = LAND
        self.dead_list = []
        for (row, col), _owner in self.hill_list.items():
            self.map[row][col] = LAND
        self.hill_list = {}
    ...
    def issue_order(self, order):
        sys.stdout.write(f"o {order[0]} {order[1]} {order[2]}\n")
        sys.stdout.flush()
    ...
    def closest_unseen(self, row1, col1, filter=None):
        ...
        for row in range(self.height):
            for col in range(self.width):
                if (filter is None or (row, col) not in filter) and (
