...
class Ants:
    ...
    def time_remaining(self):
        return self.turntime - int(1000 * (time.perf_counter() - self.turn_start_time))

    def issue_order(self, order):
        "issue an order by writing the proper ant location and direction"
        (row, col), direction = order
        sys.stdout.write(f"o {row} {col} {direction}\n")
        sys.stdout.flush()
    ...
    def my_ants(self):
        ...
        return [
            (row, col) for (row, col), owner in self.ant_list.items() if owner == MY_ANT
        ]
