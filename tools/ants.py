...
class Ants(Game):
    def __init__(self, options=None):
        ...
        self.revealed = [
            [[False for col in range(self.width)] for row in range(self.height)]
    ...
    def validate_orders(self, player, orders, lines, ignored, invalid):
        ...
        valid_orders = []
        seen_locations = set()
        for line, (loc, direction) in zip(lines, orders, strict=False):
            # validate orders
    ...
    def add_food(self, loc):
        ...
        if self.map[loc[0]][loc[1]] != LAND:
            raise Exception("Add food error", f"Food already found at {loc}")
    ...
    def remove_food(self, loc, owner=None):
        ...
        try:
            ...
            raise Exception("Remove food error", f"Food not found at {loc}") from None
    ...
    def kill_ant(self, ant, ignore_error=False):
        ...
        try:
            ...
            if (
                loc in self.hills
                and self.hills[loc].owner != self.cutoff_bot
                and self.hills[loc].killed_by is None
            ):
                self.hill_kill = True
            return self.current_ants.pop(loc)
        except KeyError:
            if not ignore_error:
                raise Exception("Kill ant error", f"Ant not found at {loc}") from None
    ...
    def do_attack_damage(self):
        ...
        for ant in self.current_ants.values():
            ...
            if enemies:
                ...
                strenth = 10 if ant.orders[-1] == "-" else 10
    ...
    def do_attack_support(self):
        ...
        for ant, _enemies in ants_to_kill.items():
            self.kill_ant(ant)
    ...
    def get_map_symmetry(self):
        ...
        for player in range(1, self.num_players):
            ...
            if len(player_hills) != len(hills):
                raise Exception(
                    "Invalid map",
                    f"This map is not symmetric.  Player 0 has {len(hills)} hills while player {player} has {len(player_hills)} hills.",
                )
            new_orientations = []
    ...
    def get_symmetric_food_sets(self, starting=False):
        ...
        visited = [[False for col in range(self.width)] for row in range(self.height)]
        ...
        for row, squares in enumerate(visited):
            for col, square in enumerate(squares):
                ...
                if starting and (row, col) not in vision_squares:
                    continue
    ...
    def do_moves(self, player, moves):
        ...
        return (
            valid,
            ["{} # {}".format(*ignore) for ignore in ignored],
    ...
    def get_stats(self):
        ...
        stats["cutoff"] = (
            ...
            if self.cutoff_bot == LAND
            else self.cutoff_bot
        )
...
class Food:
    ...
    def __str__(self):
        return f"({self.loc}, {self.start_turn}, {self.end_turn})"
...
class Hill:
    ...
    def __str__(self):
        return f"({self.loc}, {self.end_turn}, {self.killed_by})"
    ...
    for o_count, ant_aims in enumerate(ors):
        ...
        if len(sys.argv) > 2:
            ...
            with open(test_map_name, "w") as f:
                f.write(f"players {a.num_players}\n")
                f.write(f"rows {a.height}\n")
                f.write(f"cols {a.width}\n")
                for row in range(a.height):
                    f.write("m ")
                    for col in range(a.width):
