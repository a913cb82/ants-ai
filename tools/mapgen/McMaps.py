...
def random_box():
    ...
    for box in range(randrange(7, 14)):
        left = randrange(width - 5)
        t = randrange(height - 5)
        r = randrange(left + 2, min(left + width // 4, width))
        ...
        for y in range(height - b - 1, height - t):
            for x in range(left, r + 1):
                carve(y, x)
        for y in range(t, b + 1):
            for x in range(width - r - 1, width - left):
                carve(y, x)
        for y in range(height - b - 1, height - t):
            for x in range(width - r - 1, width - left):
                carve(y, x)
        if box == 0:
            ...
            m[height - t - 1][left] = 2
            m[t][width - left - 1] = 3
            m[height - t - 1][width - left - 1] = 4
            for y in range(t + 1, height - t - 1):
                carve(y, left)
                carve(y, width - left - 1)
            for x in range(left + 1, width - left - 1):
                carve(t, x)
                carve(height - t - 1, x)
    return ant_map(m)
...
def cell_maze():
    ...
    point_count = 100
    spacing = 5
    random_points(point_count, size, spacing, euclidean_distance)
