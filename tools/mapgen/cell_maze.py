...
class CellMazeMap(Map):
    def __init__(self, options=None):
        ...
        self.players = options.get(
            "players", max(2, min(10, int((betavariate(2.5, 3.0) * 10) + 1)))
        )
        self.area = options.get(
            "area", randrange(900 * self.players, min(25000, 5000 * self.players))
        )
        self.cell_width = options.get("cell_width", min(paretovariate(2), 7.0))
        self.cell_size = options.get(
            "cell_size",
            min(
                paretovariate(2) + max(5.0 + self.cell_width, self.cell_width * 2), 20.0
            ),
        )
        ...
        self.maze_type = options.get(
            "maze_type", choice(["prims", "backtrack", "growing"])
        )
        self.v_sym = options.get("v_sym")
        self.v_step = options.get("v_step")
        self.h_sym = options.get("h_sym")
        self.h_step = options.get("h_step")
        self.hills = options.get("hills")
        self.grandularity = options.get("grandularity", 1)

        self.report(f"players {self.players}")
        self.report(
            f"area {self.area}, {self.area // self.players} ({sqrt(self.area / self.players):.1f}^2) per player"
        )
        self.report(f"cell width: {self.cell_width}")
        self.report(f"cell size: {self.cell_size}")
        self.report(f"openness: {self.openness}")
        if self.grid is not None:
            self.report(f"grid: {self.grid}")
        self.report(f"maze type {self.maze_type}")
        if self.v_sym is not None:
            self.report(f"vertical symmetry: {self.v_sym}")
        if self.v_step is not None:
            self.report(f"vertical shear step: {self.v_step}")
        if self.h_sym is not None:
            self.report(f"horizontal symmetry: {self.h_sym}")
        if self.h_step is not None:
            self.report(f"horizontal shear step: {self.h_step}")
        if self.hills is not None:
            self.report(f"hills per player: {self.hills}")
    ...
    def generate(self):
        ...
        def pick_size(grid=None, aspect_ratio=None):
            ...
            if aspect_ratio is None:
                aspect_ratio = (
                    (random() * (max_val - min_val) + min_val) * col_sym / row_sym
                )
        ...
        def random_points(size, spacing, grandularity=1, count=1000):
            ...
            matrix = {
                row: list(range(0, cols, grandularity))
        ...
        def make_symmetric(
            ...
            def both_point(point, size, step, funcs):
                ...
                return (
                    vert_func(point[0], size[0], step),
                    horz_func(point[1], size[1], step),
                )
            ...
            def extend(funcs, points, size, count=2, step=0, shear=1):
                ...
                for sym_points in points:
                    if type(sym_points) is not list:
            ...
            if row_sym > 1:
                if v_sym is None:
                    if row_sym % 2 == 0 and random() < 0.5 and row_sym <= 6:
                        v_sym = (
                            choice((vert_mirror, vert_rotate))
                            if row_sym < 6
                            else vert_rotate
                        )
                    else:
                        step = choice(sym_step[row_sym]) if v_step is None else v_step
                        v_sym = vert_shear
                else:
                    v_sym = [
                        key
                        for key, value in extend_report.items()
                        if value == "vert_" + v_sym
                    ][0]
                    if v_sym == vert_shear:
                        step = choice(sym_step[row_sym]) if v_step is None else v_step
                    else:
                        step = 0
                if step > 0:
                    self.report(f"vert shear step: {step}")
                ...
                if step > 0:
                    ...
                    if h_sym is None:
                        if col_sym % 2 == 0 and random() < 0.5 and col_sym <= 6:
                            h_sym = (
                                choice((horz_mirror, horz_rotate))
                                if col_sym < 6
                                else horz_rotate
                            )
                        else:
                        ...
                        if h_sym == horz_shear:
                            ...
                            step = 0
                if step > 0:
                    self.report(f"horz shear step: {step}")
        ...
        def build_paths(points, size, distance):
            ...
            for row in range(rows):
                for col in range(cols):
                    ...
                    closest = [
                        point_comp for point_comp, d in distances.items() if d <= cutoff
                    ]
                    comps = tuple(sorted({comp for point, comp in closest}))
                    if len(closest) > 1 and len(comps) > 1:
        ...
        def fuse_cells(paths, cell1, cell2):
            ...
            self.log(f"fuse {cell1} and {cell2}")
            ...
            for path in list(paths.keys()):
                if cell2 in path:
                    if cell1 in path:
                        ...
                        new_path = tuple(
                            sorted(cell1 if n == cell2 else n for n in path)
                        )
                    ...
                    if len(path) == 2 and not any(
                        ...
                        if neighbor_dist(neighbors, path[0], path[1]) is not None:
                            ...
                            for triple in list(paths.keys()):
                                if (
                                    len(set(triple)) == 3
                                    and path[0] in triple
                                    and path[1] in triple
                                ):
        ...
        def carve(paths, path, ilk=LAND, inclusive=False):
            ...
            if inclusive:
                path = [p for p in paths if len(set(path) - set(p)) == 0]
        ...
        def ensure_connected(paths, carved_paths, carve):
            ...
            if len(sections) > 1:
                self.log(f"connecting maze, sections: {len(sections)}")
                # look for largest blocks of squares first, since this will make it easiest to traverse
                for path in (
                    path
                    for _length, path in sorted(
                        (-len(locs), path)
                        for path, locs in paths.items()
                        if len(path) == 2 and path not in carved_paths
                    )
                ):
                    carve(path, LAND)
                    new_sections = self.section(0)
                    if len(new_sections) < len(sections):
                        self.log(f"maze connected, sections: {len(new_sections)}")
                        ...
                        if len(block_sections) > len(new_sections):
                            self.log(
                                f"opening connection, sections: {len(block_sections)}"
                            )
                            # carved path not wide enough
                            for third in neighbor:
                                ...
                                if ac in paths and bc in paths:
                                    for p in (ac, bc, abc):
                                        if p not in carved_paths:
                                            carve(p, LAND)
                                    # log('# found %s' % (tuple(sorted([third, path[0], path[1]])),))
                                    new_block_sections = [
                                        s for s in self.section(1) if len(s[0]) > 9
                                    ]
        ...
        def growing_tree(nodes, carve, visited=None):
            if visited is None:
                visited = []
            visited = set(visited)
            carved_paths = []
            ...
            maze_type = [f for f, t in maze_type_report.items() if t == self.maze_type][
                0
            ]
        ...
        def set_openness(
            points, braids, carved_paths, openness, carve, open_braids=None
        ):
            if open_braids is None:
                open_braids = []
            ...
            self.log(f"set openness to {self.openness} on {len(braids)} braids")
            ...
            while valid_first_count == 0:
                ...
                for i, s_points in enumerate(points):
                    hill_dists = [
                        dist
                        for _, dist in self.get_distances(s_points[0], s_points[1:])
                    ]
                    if len(hill_dists) == 0:
                        ...
                        if (
                            min_count > 0
                            or stdev(hill_dists) < 2.0 * max(hill_dists) / self.players
                        ):
                            valid_first_count += 1
                        else:
            ...
            if adjusted:
                self.log(f"adjust openness to {self.openness}")
        ...
        def make_euclidean_distance(size):
            ...
            dist_table = [
                [
                    sqrt(min(y, cols - y) ** 2 + min(x, rows - x) ** 2)
                    for y in range(cols)
                ]
                for x in range(rows)
            ]
            ...
            for r in range(size[0]):
                for c in range(size[1]):
                    for i, sym_points in enumerate(points):
                        ...
                        fd.write(
                            TILE[(r // tile_size[0] + c // tile_size[1]) % len(TILE)]
                        )
                fd.write("\n")
            fd.write("\n")
        ...
        def cell_maze():
            ...
            sym_points, sym_size = make_symmetric(
                points,
                tile_size,
                self.players,
                grid,
            ...
            self.report(f"area: {sym_size[0] * sym_size[1]}")
            self.report(f"grid size: {grid}")
            self.report(f"tile size: {tile_size} {1.0 * tile_size[1] / tile_size[0]}")
            self.report(
                f"map size: {(sym_size[0], sym_size[1])} {1.0 * sym_size[1] / sym_size[0]}"
            )
            self.log(f"point count: {len(points)}")
            ...
            neighbors = build_neighbors(paths.keys())
            paths, neighbors, narrow_paths = remove_narrow_paths(
                paths, neighbors, sym_points, sym_size
            )
            ...
            #                                           lambda path: carve(paths, path, LAND))
            carved_paths.extend(
                growing_tree(neighbors, lambda path: carve(paths, path))
            )
            carved_paths = ensure_connected(
                paths, carved_paths, lambda path, ilk: carve(paths, path, ilk)
            )

            braids = [
                path
                for path in paths
                if len(path) == 2 and path not in narrow_paths + carved_paths
            ]
            mirror_braids = sorted(
                [path for path in paths if len(path) == 2 and path[0] == path[1]],
                key=lambda path: len(paths[path]),
            )
            braid_paths, valid_hills = set_openness(
                sym_points,
                braids,
                carved_paths,
                self.openness,
                lambda path, ilk: carve(paths, path, ilk),
                mirror_braids,
            )
            carved_paths.extend(braid_paths)

            valid_hills.sort(
                key=lambda valid_hill: (valid_hill[2], -max(valid_hill[1]))
            )
            comp, hill_dists, _ = valid_hills.pop(0)
            first_hills = sym_points[comp]
            self.log(f"first hill stdev: {stdev(hill_dists)}")
            ...
            while len(valid_hills) > 0:
                if (hill_count is None and random() < 0.5) or hill_count > 0:
                    ...
                    enemies = {
                        self.map[row][col]
                        for (row, col), dist in hill_dists
                    ...
                    for player, (row, col) in enumerate(hills):
                        if player == 0:
                            ...
                            p_row, p_col = self.dest_offset(
                                (row, col),
                                self.offset_aim(hill_offset, map_sym[player][1]),
                                sym_size,
                            )
...
def main():
    parser = OptionParser()
    parser.add_option("-s", "--seed", dest="seed", type="int", help="random seed")
    parser.add_option(
        "-p", "--players", dest="players", type="int", help="number of players"
    )
    parser.add_option(
        "-o",
        "--openness",
    ...
    parser.add_option("-a", "--area", dest="area", type="int", help="area of map")
    parser.add_option(
        "-w",
        "--cell_width",
        dest="cell_width",
    ...
    parser.add_option(
        "-c", "--cell_size", dest="cell_size", type="float", help="cell size"
    )
    parser.add_option(
        "-g",
        "--grid",
    ...
    parser.add_option(
        "--v_step", dest="v_step", type="int", help="vertical shearing step"
    )
    parser.add_option(
        "--h_sym",
        dest="h_sym",
    ...
    parser.add_option(
        "--h_step", dest="h_step", type="int", help="horizontal shearing step"
    )
    parser.add_option(
        "-r",
        "--ratio",
    ...
    parser.add_option("--hills", dest="hills", type="int", help="hills per player")
    parser.add_option(
        "--grandularity",
        dest="grandularity",
        type="int",
