...
class SymmetryException(Exception):
    ...
    def __str__(self):
        return f"This symmetry type only supports {self.msg}."
...
class SymmetricMap(Map):
    def __init__(self, **kwargs):
        ...
        if symmetry_type not in symmetry_types:
            raise Exception(f"Unknown symmetry type {symmetry_type}")
        ...
        if symmetry_type == "translational":
            ...
            self.translation = Point(
                self.size.x / num_players * self.translation_factor.x,
                self.size.y / num_players * self.translation_factor.y,
            )
    ...
    def add_hill(self, location):
        ...
        for player_id, loc in enumerate(self.symmetry_vector(location)):
    ...
    def vector_point(self, origin):
        ...
        if len(self.players) >= 2:
            points.append(
                Point(self.size.x - origin.x - 1, self.size.y - origin.y - 1)
            )  # vertical/horizontal(point) reflection

        if len(self.players) >= 4:
            if self.size.x != self.size.y:
                raise SymmetryException("square maps")
            points.append(
                Point(self.size.x - origin.x - 1, origin.y)
            )  # vertical reflection
            points.append(
                Point(origin.x, self.size.y - origin.y - 1)
            )  # horizontal reflection

        if len(self.players) >= 8:
            points.append(
                Point(self.size.x - origin.y - 1, self.size.y - origin.x - 1)
            )  # diagonal reflection to origin
            points.append(
                Point(origin.y, origin.x)
            )  # diagonal reflection to point reflection
            points.append(
                Point(origin.y, self.size.y - origin.x - 1)
            )  # diagonal reflection to vertical reflection
            points.append(
                Point(self.size.x - origin.y - 1, origin.x)
            )  # diagonal reflection to horizontal reflection
    ...
    def vector_translational(self, origin):
        return [
            (origin + self.translation * playerid).normalize(self.size)
...
symmetry_types = {
    function[len("vector_") :]
    for function in dir(SymmetricMap)
    if function.startswith("vector_")
}
