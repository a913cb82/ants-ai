...
class Grid:
    ...
    def pick_tile_dimensions(self):
        ...
        for _d_attempt in range(200000):
    ...
    def pick_rotational_dimensions(self):
        ...
        for _d_attempt in range(100):
            ...
            for _a_attempt in range(2 * self.rows):
                while True:
                    self.h_loc = self.random_loc()
    ...
    def get_adjacent_block(self, block, direction):
        for _n in range(int(math.sqrt(len(block)))):
    ...
    def add_extra_hills(self):
        ...
        for _h in range(self.no_hills - 1):
            for _d_attempt in range(100):
    ...
    def add_extra_land_blocks(self):
        ...
        for _extra_loc in range(extra_locs):
            block_found = False
            for _b_attempt in range(100):
...
def main(argv):
    ...
    if opts.symmetry == "rotational":
        if (
            ...
            return
        if opts.rotational_symmetry != -1 and (
            (
                opts.no_players == 2
    ...
    elif grid.symmetry == "tile" and not grid.tile_symmetric_grid(
        opts.no_players,
        opts.min_dimensions,
        opts.max_dimensions,
        opts.min_starting_distance,
