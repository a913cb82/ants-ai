    ...
    try:
        ...
        for b, bot in enumerate(botcmds):
            ...
            if not sandbox.is_alive:
                ...
                bot_turns[-1] = 0
                if verbose_log:
                    verbose_log.write(f"bot {b} did not start\n")
                game.kill_player(b)
            sandbox.pause()
        ...
        if verbose_log:
            verbose_log.write(f"running for {turns} turns\n")
        for turn in range(turns + 1):
            ...
            time_limit = loadtime if turn == 0 else turntime
            ...
            for group_num in range(0, len(bot_list), simul_num):
                pnums, pbots = zip(
                    *bot_list[group_num : group_num + simul_num], strict=False
                )
                ...
                for b, moves in enumerate(bot_moves):
                    if game.is_alive(b):
                        ...
                        if output_logs and output_logs[b]:
                            output_logs[b].write(f"# turn {turn}\n")
                            if valid and output_logs and output_logs[b]:
                                output_logs[b].write("\n".join(valid) + "\n")
                                output_logs[b].flush()
                        if ignored:
                            if error_logs and error_logs[b]:
                                error_logs[b].write(
                                    f"turn {turn:4d} bot {b} ignored actions:\n"
                                )
                                error_logs[b].write("\n".join(ignored) + "\n")
                                error_logs[b].flush()
                            if output_logs and output_logs[b]:
                        ...
                        if invalid:
                            ...
                            if error_logs and error_logs[b]:
                                error_logs[b].write(
                                    f"turn {turn:4d} bot {b} invalid actions:\n"
                                )
                                error_logs[b].write("\n".join(invalid) + "\n")
                                error_logs[b].flush()
                            if output_logs and output_logs[b]:
            ...
            for b in bots_eliminated:
                if verbose_log:
                    verbose_log.write(f"turn {turn:4d} bot {b} eliminated\n")
            ...
            if bots_eliminated and end_wait:
                if verbose_log:
                    verbose_log.write(
                        f"waiting {end_wait} seconds for bots to process end turn\n"
                    )
                time.sleep(end_wait)
            for b in bots_eliminated:
                bots[b].kill()
            ...
            if verbose_log:
                ...
                for key in stat_keys:
                    ...
                    verbose_log.write(
                        " {0:^{1}}".format(str(values), max(len(key), len(str(values))))
                    )
                verbose_log.write("\n")
        ...
        game.finish_game()
        score_line = "score {}\n".format(" ".join(map(str, game.get_scores())))
        status_line = "status {}\n".format(" ".join(bot_status))
        status_line += "playerturns {}\n".format(" ".join(map(str, bot_turns)))
        end_line = f"end\nplayers {len(bots)}\n" + score_line + status_line
        if stream_log:
