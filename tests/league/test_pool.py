...
def make_repo(path: Path, layout: dict[str, str]) -> Path:
    ...
    subprocess.run(["git", "-C", str(path), "commit", "-qm", "bots"],
                   check=True)
    return path
...
def test_two_manifests_one_dir_are_two_bots(tmp_path):
    from pool import pool
    r = make_repo(tmp_path / "r", {"bots/a/slow.bot": "python A.py --slow\n",
                                   "bots/a/fast.bot": "python A.py --fast\n",
                                   "bots/a/A.py": "x\n"})
    ids = pool(r)
    assert len(ids) == 2
    assert {i.split("-")[0] for i in ids} == {"bots/a/slow.bot",
                                              "bots/a/fast.bot"}


def test_identical_dup_manifests_collapse(tmp_path):
    from pool import pool
    r = make_repo(tmp_path / "r", {"bots/a/one.bot": "python A.py\n",
...
def test_code_change_rehashes(tmp_path):
    ...
    subprocess.run(["git", "-C", str(r), "commit", "-qam", "tweak"],
                   check=True)
...
def test_choose_canonical_prefers_rated():
    ...
    games_of = {("bots/x/main.bot", "newsha"): 0,
                ("bots/x/main.bot", "oldsha"): 5}
    order = {"newsha": 1, "oldsha": 0}
    assert choose_canonical(specs, games_of, order) == \
        ("bots/x/main.bot", "oldsha")
...
def test_command_text_is_verbatim(tmp_path):
    from pool import pool, bot_cmd
    r = make_repo(tmp_path / "r", {"bots/a/main.bot": "python A.py --x 1\n",
                                   "bots/a/A.py": "x\n"})
    assert bot_cmd(r, pool(r)[0]) == "python A.py --x 1"


def test_nested_manifests_found(tmp_path):
    from pool import pool
    r = make_repo(tmp_path / "r", {"tools/sample/x.bot": "run x\n",
                                   "tools/sample/x.py": "x\n"})
    assert len(pool(r)) == 1
